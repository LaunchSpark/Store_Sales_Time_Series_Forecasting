...
make_features <- function(DT) {
  ...
  # Convert to factors so downstream models retain categorical structure
  DT[, `:=`(
...
build_preprocessor <- function(train_predictors) {
  train_factors <- train_predictors %>%
    mutate(across(
      ...
      ~ forcats::fct_explicit_na(factor(.x), na_level = "Missing")
    ))
...
apply_preprocessor <- function(df, preproc) {
  df_mod <- df %>%
    mutate(across(
      all_of(preproc$numeric_keep),
      ~ replace_na(.x, preproc$numeric_means[cur_column()])
    )) %>%
    mutate(across(
      all_of(categorical_cols),
      ~ forcats::fct_explicit_na(
        factor(.x, levels = preproc$cat_levels[[cur_column()]]),
        na_level = "Missing"
      )
    ))

  if (isTRUE(preproc$scale_numeric)) {
    df_mod <- df_mod %>%
      mutate(across(
        all_of(preproc$numeric_keep),
        ~ (.x - preproc$numeric_means[cur_column()]) / preproc$numeric_sds[cur_column()]
      ))
  }

  df_mod %>%
    select(any_of("y"), all_of(preproc$numeric_keep), all_of(categorical_cols)) %>%
    as_tibble() %>%
...
pred_valid <- predict(rf_fit, data = valid_processed %>% select(-y))$predictions
...
Because categorical variables are kept as factors, the importance table
reports aggregate signals per predictor (e.g., `family` rather than
level-specific dummies), and those names are reused when trimming
features for refits and submissions.
...
set.seed(42)
rf_top_fit <- ranger(
  x = train_top_processed %>% select(-y),
  y = train_top_processed$y,
...
pred_valid_top <- predict(rf_top_fit, data = valid_top_processed %>% select(-y))$predictions
...
Establish a baseline for us to compare against
```{r baseline}
baseline_predictor <- mean(train_processed$y, na.rm = TRUE)
baseline_rmse <- yardstick::rmse_vec(truth = valid_processed$y, estimate = rep(baseline_predictor, nrow(valid_processed))
)
