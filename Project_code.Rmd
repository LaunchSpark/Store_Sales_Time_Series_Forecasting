...
stopifnot(nrow(trans_u) == trans_u[, uniqueN(date, store_nbr)])
...
build_preprocessor <- function(train_predictors) {
  ...
  train_design <- model.matrix(feature_formula, data = scaled_train)
  col_var <- apply(train_design, 2, stats::var)
...
apply_preprocessor <- function(df, preproc) {
  ...
  design <- model.matrix(preproc$feature_formula, data = df_mod)
