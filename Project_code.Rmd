...
To ground expectations we fit classic univariate baselines—ARIMA and
ETS—on a representative store-family series. Measuring their 16-day
forecast accuracy gives us a benchmark to beat with more expressive
global models.
...
train_series <- as_tibble(train_fit) %>%
  filter(store_nbr == baseline_store, family == baseline_family) %>%
  arrange(date)

valid_series <- as_tibble(valid_fit) %>%
  filter(store_nbr == baseline_store, family == baseline_family) %>%
  arrange(date)

train_sales <- train_series %>%
  filter(!is.na(sales)) %>%
  pull(sales)

valid_sales <- valid_series %>%
  filter(!is.na(sales)) %>%
  pull(sales)

stopifnot(length(valid_sales) >= H)

ts_train <- ts(train_sales, frequency = 7)
fit_arima <- forecast::auto.arima(ts_train)
fit_ets   <- forecast::ets(ts_train)

fc_arima <- forecast::forecast(fit_arima, h = H)$mean
fc_ets   <- forecast::forecast(fit_ets, h = H)$mean

actual_lastH <- tail(valid_sales, H)

tibble(
  model = c("ARIMA", "ETS"),
  RMSE  = c(
    sqrt(mean((fc_arima - actual_lastH)^2, na.rm = TRUE)),
    sqrt(mean((fc_ets   - actual_lastH)^2, na.rm = TRUE))
  )
)
```

These two baselines are intentional rather than redundant. **ARIMA**
provides an autoregressive, differenced view of the demand history that
captures shocks tied to promotions or holidays. **ETS**, on the other
hand, emphasizes exponentially weighted trend/seasonality components and
is a better stress test for smoother families where level shifts matter
more than sudden spikes. Keeping both reveals whether a single
store-family behaves more like a volatile, promotion-driven series or a
stable, seasonal one; if their errors converged we would drop one, but in
practice we see complementary failure modes worth tracking.

# 7) Global Supervised Model: Random Forest (with Recipe)

With single-series baselines in place, we move to a global model that can
...
* Consider stacking (ARIMA/ETS + Random Forest) or trying `{lightgbm}` / `{catboost}`.
