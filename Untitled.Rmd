...
hol_clean <- as_tibble(hol) %>%
  mutate(is_holiday_raw = 1L,
         is_transferred = if_else(transferred == "True", 1L, 0L, missing = 0L)) %>%
  transmute(date, is_holiday = if_else(is_transferred==1L, 0L, 1L))
...
join_base <- function(df_dt) {
  df <- as_tibble(df_dt) %>%
    left_join(as_tibble(stores), by = "store_nbr") %>%
    left_join(oil_full, by = "date") %>%
    left_join(hol_clean, by = "date") %>%
    left_join(as_tibble(trans), by = c("date","store_nbr")) %>%
    clean_names()
...
make_features <- function(DT) {
  DT[, `:=`(
    ...
    month = month(date),
    mday = mday(date),
    wday = wday(date),                  # numeric 1..7
    week = isoweek(date),
    is_weekend = as.integer(wday %in% c(1,7))  # Sunday=1
  )]
